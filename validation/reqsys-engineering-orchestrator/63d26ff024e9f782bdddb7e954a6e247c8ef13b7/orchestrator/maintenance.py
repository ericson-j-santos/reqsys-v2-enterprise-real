...
VALID_HOST_PROFILES = {"NORMAL", "ESTUDO"}
...
def set_host_profile(
    *,
    ...
    if requested not in VALID_HOST_PROFILES:
        raise MaintenanceError("profile must be NORMAL or ESTUDO")
    correlation = str(correlation_id or "").strip()
    if not 8 <= len(correlation) <= 128 or "\r" in correlation or "\n" in correlation:
        raise MaintenanceError("correlation_id invalid")
    if profile_path is None:
        raise MaintenanceError("profile_path unavailable")

    path = Path(profile_path)
    if not path.parent.is_dir():
        raise MaintenanceError("profile directory unavailable")
    ...
    if path.exists():
        try:
            ...
            raise MaintenanceError("existing host profile invalid") from exc
        if not isinstance(before, dict):
            raise MaintenanceError("existing host profile invalid")
        observed_host = str(before.get("host") or local_host).strip()
        if observed_host.casefold() != local_host.casefold():
            raise MaintenanceError("existing host profile belongs to another host")
        before_profile = str(before.get("profile") or "").strip().upper()
        if before_profile not in VALID_HOST_PROFILES:
            raise MaintenanceError("existing host profile invalid")

    changed = before_profile != requested
    if changed:
        atomic_json(
            path,
            {
    ...
    try:
        after = json.loads(path.read_text(encoding="utf-8")) if path.exists() else {}
    except (OSError, json.JSONDecodeError) as exc:
        raise MaintenanceError("host profile readback invalid") from exc
    observed_profile = str(after.get("profile") or before_profile).strip().upper()
    observed_host = str(after.get("host") or local_host).strip()
    if observed_profile != requested or observed_host.casefold() != local_host.casefold():
        raise MaintenanceError("host profile independent readback mismatch")

    return {
        "handler": PROFILE_SET_TASK,
        "host": local_host,
        "profile": requested,
        "accepts_new_development": requested == "NORMAL",
        "changed": changed,
