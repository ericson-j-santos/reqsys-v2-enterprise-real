...
def wait_for(url: str, predicate, timeout_seconds: int = 120) -> dict:
...
def main() -> None:
    wait_for("http://127.0.0.1:8000/health", lambda p: p.get("status") == "healthy")
    wait_for("http://127.0.0.1:9090/-/ready", lambda _: True)
    wait_for("http://127.0.0.1:9093/-/ready", lambda _: True)
    wait_for("http://127.0.0.1:3000/api/health", lambda p: p.get("database") == "ok")
    ...
    alerts = get_json("http://127.0.0.1:9090/api/v1/rules")
    groups = alerts.get("data", {}).get("groups", [])
    if not groups:
