...
WORKFLOW = ROOT / ".github/workflows/worker-pool-portable-contract-e2e.yml"
...
def test_portable_e2e_workflow_is_github_hosted_sha_pinned_and_secretless() -> None:
    ...
    assert "repository: ericson-j-santos/engineering-worker-pool" in raw
    assert f"ref: {WORKER_POOL_SHA}" in raw
    assert f"WORKER_POOL_SHA: {WORKER_POOL_SHA}" in raw
    assert f"RULES_SHA: {RULES_SHA}" in raw
    assert "permissions:\n  contents: read" in raw
    assert "secrets." not in raw
    assert "workflow_dispatch:" in raw
    assert "push:" in raw
    assert "timeout-minutes: 8" in raw
    assert "worker_pool_portable_contract_e2e.py" in raw
    assert "deploy" not in raw.lower().replace("deploy_executed", "").replace("deploy executado", "")
...
def test_portable_e2e_is_registered_as_issue_2020_gap_without_replacing_physical_gate() -> None:
    ...
    assert "worker-pool-portable-contract-e2e.yml" in spec
