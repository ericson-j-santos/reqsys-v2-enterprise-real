...
POLICY_CONTRACT = ROOT / "docs" / "ops-dashboard" / "data" / "autonomous-delivery-cycle-policy.json"
...
def test_autonomous_delivery_policy_requires_governed_labels() -> None:
    payload = json.loads(POLICY_CONTRACT.read_text(encoding="utf-8"))

    assert payload["max_prs_per_cycle"] == 1
    assert "cycle:auto-merge-approved" in payload["required_labels"]
    assert "merge-queue:eligible" in payload["required_labels"]
    assert payload["next_increment_queue"]["mode"] == "report_only"
