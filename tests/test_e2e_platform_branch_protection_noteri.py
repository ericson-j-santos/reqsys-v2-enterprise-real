...
def test_apply_job_uses_noteri_session_launcher_and_risk3() -> None:
    ...
    assert "reqsys.e2e-platform-main-protection.dev" in raw
    assert "repo://ericson-j-santos/e2e-platform/branch/main" in raw
...
def test_local_protection_script_is_dynamic_fail_closed_and_sanitized() -> None:
    ...
    assert 'TARGET_REPOSITORY = "ericson-j-santos/e2e-platform"' in raw
    assert 'TARGET_BRANCH = "main"' in raw
    assert 'REQUIRED_CHECK = "contract-self-test"' in raw
