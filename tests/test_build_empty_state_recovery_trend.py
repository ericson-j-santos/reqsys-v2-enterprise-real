...
class EmptyStateRecoveryTrendTests(unittest.TestCase):
    def test_marks_ux_100_ready_with_objective_criteria(self):
        events = []
        ...
        self.assertEqual(report["median_recovery_seconds"], 20.0)
    ...
    def test_generates_advisory_alerts_below_targets(self):
        report = evaluate([
            {"context": "analytics", "event": "view", "occurredAt": "2026-07-15T10:00:00Z"},
            {"context": "analytics", "event": "primary_action", "occurredAt": "2026-07-15T10:00:45Z"},
            {"context": "analytics", "event": "view", "occurredAt": "2026-07-15T10:01:00Z"},
        ])
        codes = {alert["code"] for alert in report["alerts"]}
        self.assertIn("UX_RECOVERY_SAMPLE_INSUFFICIENT", codes)
        self.assertIn("UX_RECOVERY_RATE_BELOW_TARGET", codes)
        self.assertIn("UX_RECOVERY_MEDIAN_ABOVE_TARGET", codes)
    ...
    def test_idempotent_dashboard_and_history_limit(self):
        ...
        self.assertEqual(history[-1]["median_recovery_seconds"], 10.0)
        self.assertIn("UX_RECOVERY_SAMPLE_INSUFFICIENT", history[-1]["alert_codes"])
