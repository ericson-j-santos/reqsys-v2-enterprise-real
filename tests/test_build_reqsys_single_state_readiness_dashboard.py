...
def test_dashboard_renders_consumers_and_guardrails() -> None:
    dashboard = build_dashboard(sample_report())
    assert all(name in dashboard for name in ("Governance", "Runtime", "Analytics"))
    assert "100.00%" in dashboard
    assert "NÃO BLOQUEANTE" in dashboard
    assert "automatic_promotion_allowed=false" in dashboard


def test_dashboard_escapes_dynamic_text() -> None:
