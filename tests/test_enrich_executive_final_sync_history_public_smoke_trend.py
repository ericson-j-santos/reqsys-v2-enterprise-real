...
from scripts.enrich_executive_final_sync_history_public_smoke_trend import (
    CARD_KEY,
    build_state,
    enrich,
)
...
def history(environment: str, count: int = 3, passed: bool = True, synchronized: bool = True, fingerprint: str = "fp-1") -> dict:
    status = "PUBLIC_FINAL_HISTORY_SMOKE_OK" if passed else "PUBLIC_FINAL_HISTORY_SMOKE_REVIEW"
    return {
        "samples": [
            {
...
class ExecutiveFinalSyncHistoryPublicSmokeTrendTest(unittest.TestCase):
    def test_complete_evidence_is_only_human_review_eligible(self):
        ...
        self.assertEqual("fp-1", state["common_fingerprint"])
        self.assertFalse(state["production_blocker"])

    def test_missing_environment_is_insufficient(self):
    ...
    def test_different_fingerprints_detect_drift(self):
    ...
    def test_review_status_requires_attention(self):
        histories = {name: history(name) for name in ("dev", "stg", "prod")}
