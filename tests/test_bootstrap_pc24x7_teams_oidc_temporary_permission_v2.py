...
SCRIPT = Path('scripts/bootstrap_pc24x7_teams_oidc_temporary_permission_v2.py')
SPEC = importlib.util.spec_from_file_location('bootstrap_pc24x7_teams_oidc_temporary_permission_v2', SCRIPT)
...
def test_non_owner_bloqueia_sem_ownedby(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_falha_workflow_ainda_revoga(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(MODULE, '_resolve_context', lambda *_: MODULE.Context('tenant-dev', 'app', 'sp', 'graph'))
    monkeypatch.setattr(MODULE, '_owners', lambda *_: ['sp'])
...
def test_dry_run_nao_concede(monkeypatch: pytest.MonkeyPatch) -> None:
    args = _args()
    args.dry_run = True
    monkeypatch.setattr(MODULE, '_resolve_context', lambda *_: MODULE.Context('tenant-dev', 'app', 'sp', 'graph'))
    monkeypatch.setattr(MODULE, '_owners', lambda *_: ['sp'])
