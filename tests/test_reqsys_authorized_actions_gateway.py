...
def test_gateway_restringe_issue_ator_e_comandos_exatos() -> None:
    ...
    assert "github.event.comment.body == '/reqsys run pc24x7-worker-pool-reconcile-dev'" in content
...
def test_gateway_usa_allowlist_estatica_sem_workflow_arbitrario() -> None:
    ...
    assert "target='pc24x7-worker-pool-reconcile-dev.yml'" in content
    ...
    assert (
        ...
        "pc24x7-worker-pool-reconcile-dev.yml|"
...
def test_gateway_worker_pool_reconcile_dev_is_exact_inputless_and_fail_closed() -> None:
    content = _workflow()

    assert "github.event.comment.body == '/reqsys run pc24x7-worker-pool-reconcile-dev'" in content
    assert "'/reqsys run pc24x7-worker-pool-reconcile-dev')" in content
    assert "target='pc24x7-worker-pool-reconcile-dev.yml'" in content
    assert "steps.route.outputs.target == 'pc24x7-worker-pool-reconcile-dev.yml'" in content
    assert "SELF_HOSTED_RUNNER_PICKUP_TIMEOUT_OR_BUSY" in content
    assert "-f service=" not in content
