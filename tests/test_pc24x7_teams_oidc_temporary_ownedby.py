...
def _ctx() -> object:
    return MODULE.Context('tenant-dev', 'app', 'sp', 'graph')


def test_non_owner_falha_fechado_sem_escalada(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_ownedby_preexistente_nao_e_revogado(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(MODULE, '_resolve_context', lambda *_: _ctx())
...
def test_dry_run_nao_muta(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(MODULE, '_resolve_context', lambda *_: _ctx())
    monkeypatch.setattr(MODULE, '_owners', lambda *_: ['sp'])
...
def test_contrato_workflow_dev_sem_escalada() -> None:
