...
- Endpoints de métricas Prometheus text/plain para o Connection Broker em `/api/connectors/metrics` e `/v1/connectors/metrics`.
- Testes xUnit dedicados para validar famílias de métricas exportadas pelo Connection Broker.
...
- `Program.cs`: registro dos endpoints de métricas do Connection Broker.
...
- Evoluir o Connection Broker para health-check real por provedor.
- Persistir auditoria em storage durável externo ao processo.

### Ambiente

- Ambiente observado: GitHub / branch `main`.
- Ambiente de aplicação: branch `feature/connection-broker-metrics`.
