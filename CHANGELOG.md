...
### Corrigido (GitOps · required status checks da `main` referenciavam nome de workflow, não check run name)

- Gap canônico `OPS-GAP-GITOPS-CHECKS-001` (P0 do backlog Pareto enterprise). A linha base de `docs/governance/branch-protection-enterprise-baseline.md` recomendava seis contextos obrigatórios — `Governança Padrão Ouro`, `Governance Quality Gates`, `CI — ReqSys v2 Enterprise`, `CI Enterprise Fast`, `Branch Protection Audit` e `Governed Merge Queue` — e todos os seis são **nomes de workflow**. Branch protection compara o contexto com o check run name publicado no SHA do PR, que é o `name` do job (ou o job id quando o job não declara `name`). Aplicar a linha base literalmente travaria todo PR em `Expected — Waiting for status to be reported`, sem falha visível e sem caminho de merge.
- `scripts/build_required_checks_stale_inventory.py` (novo): cruza os contextos exigidos — payload real de `required_status_checks` quando há permissão, ou a linha base versionada — com os check run names que os 563 workflows do repositório realmente produzem, classificando cada exigência em `active`, `conditional_risk`, `not_pr_triggered`, `workflow_name_mismatch`, `renamed_candidate` ou `stale`. Leitura pura: não chama API, não altera proteção (`automatic_branch_protection_change_allowed=false`) e não toca produção.
- A classificação considera o que de fato impede um check de materializar: filtro de paths no `pull_request`, filtro de branches que exclui a branch alvo, job matricial (o GitHub sufixa o contexto com os valores da matriz), nome de job dinâmico, reusable call (o contexto vira `job pai / job chamado`) e `if` condicional. `if: always()` não é risco — o job reporta — e por isso `CI Router Result`, `Sumario CI Enterprise Fast` e `Gate de merge governado` entram na lista recomendada.
- O inventário existente (`Required Checks Materialization Inventory`) amostra apenas PRs já mergeados, que por definição satisfizeram os checks obrigatórios; ele não vê contexto obsoleto ou impossível de materializar. Esse ponto cego é o que este incremento fecha.
- `config/required-checks-inventory-policy.json` (novo) separa o que está declarado hoje (`declared_required_contexts`), a lista alvo comprovada (`recommended_required_contexts`) e a decisão registrada por contexto divergente (`rename`, com `target_contexts`, `rationale`, `owner` e `decided_at`). `recommended_required_contexts` funciona como controle positivo: o contrato falha se algum contexto recomendado deixar de materializar — por exemplo, depois de alguém renomear um job.
- `config/operational-gaps-registry.json`: os seis gaps do backlog Pareto canônico passam a estar espelhados no registry lido pelo `agent_increment_gate.py`. Antes, `--increment-type gap_fix --reference OPS-GAP-GITOPS-CHECKS-001` era recusado com `gap_fix_invalido`, ou seja, a própria governança bloqueava a correção dos gaps P0 que ela declara prioritários. `tests/test_operational_gaps_registry_pareto_alignment.py` impede a divergência voltar.
- `.github/workflows/required-checks-stale-inventory.yml` (novo, report-only, classificado como `informative`): roda o contrato em PR que toque o escopo, gera o inventário diariamente e publica o artifact. `--strict` só é aplicado por `workflow_dispatch` com `strict=true`.
- Validação local: 12 testes verdes em `tests/test_build_required_checks_stale_inventory.py` (inclui `stale`, `renamed_candidate`, matriz, filtro de paths, filtro de branch, workflow não parseável, precedência do payload de proteção e fallback quando a API responde 403) e 4 em `tests/test_operational_gaps_registry_pareto_alignment.py`. Controles: sem decisão registrada o relatório fica `blocking=true`; com decisão registrada o achado permanece visível em `stale_contexts` mas deixa de bloquear. O gate de incremento foi exercitado antes (`[BLOQUEADO] gap_fix_invalido`) e depois (`[PERMITIDO] gap_fix_referenciado`), com controle negativo em referência inexistente. Suíte completa `tests/`: 2636 verdes; as 13 falhas remanescentes são idênticas em `origin/main` (Python 3.11 local e artifacts ausentes), confirmadas em worktree limpo.
- Pendência humana registrada: aplicar as decisões `rename` em `Settings > Rules > Rulesets` exige permissão administrativa. O gap só fecha quando a proteção real refletir os contextos `active` e o relatório rodar com `--strict` sem pendência.


### Adicionado (OCR · validação semântica de identificadores documentais)

- `backend/app/ocr/identificadores_documentais.py` (novo): fecha a lacuna que o próprio `docs/architecture/ocr-evidence-gate-v2.md` declarava em aberto — o gate certificava a camada OCR e os candidatos, mas não validava CPF, RG/CIN ou CNH. O módulo é stdlib-puro, não executa OCR e classifica cada identificador em `VALIDO`, `VALIDO_APOS_CORRECAO_OCR`, `INVALIDO`, `VALIDACAO_INDISPONIVEL` ou `FORMATO_INESPERADO`.
- A detecção é ancorada em rótulo (`CPF:`, `CNH REGISTRO`, `RG:`, `CIN:`) dentro de janela curta. Número solto em texto corrido não vira identificador: sem a âncora, qualquer sequência de 11 dígitos — valor monetário, protocolo, código de barras — geraria falso positivo. A correção de confusões de OCR (`O`→`0`, `S`→`5`) nunca promove a leitura a `VALIDO`, apenas a `VALIDO_APOS_CORRECAO_OCR`, com a contagem de correções registrada.
- Dois casos jamais produzem veredito: RG, que é estadual e não tem verificador nacional padronizado, e o ramo do algoritmo Denatran em que o segundo dígito da CNH ficaria negativo após o desconto — as implementações públicas divergem nele. Ambos recebem `VALIDACAO_INDISPONIVEL` em vez de um resultado arbitrário. A CIN é verificada pelo dígito do CPF, que ela adota como número único (Decreto 10.977/2022).
- LGPD: o valor lido nunca é armazenado no objeto nem serializado. A máscara revela só os dois últimos dígitos e existe apenas na aplicação — a projeção publicável em CI (`para_evidencia()`) não a carrega. O fingerprint é HMAC-SHA256 com chave (`OCR_IDENTIFIER_FINGERPRINT_KEY`), não SHA-256 puro: o espaço de 11 dígitos é enumerável (10¹¹) e um hash sem chave seria reversível por força bruta. Sem chave configurada o escopo é `EFEMERO` e correlaciona apenas dentro da execução. `requer_validacao_humana` é sempre `True` e o construtor recusa o contrário.
- `scripts/ocr_evidence_gate_v2.py` e `benchmark/ocr/evidence-v2.json`: o gate publica `identifiers`, `identifier_types`, `identifier_states` e `trusted_identifiers` por caso, e ganha três critérios — `forbid_trusted_identifiers` (identidade e conteúdo ambíguo), `expect_no_identifiers` (página vazia e documento desconhecido) e `min_identifiers`. O piso é o que impede o falso positivo estrutural: sem ele, um detector que não enxergasse nada passaria trivialmente nas proibições. No corpus sintético o rótulo passou a ficar adjacente ao número, como em documento real; os valores seguem sintéticos e deliberadamente inválidos.
- Validação ponta a ponta executada localmente com tesseract, poppler e ImageMagick reais: `OCR Evidence Gate v2` em `PASS`, 10/10 cenários, com 4 identificadores detectados no cenário de identidade (CPF, CIN, CNH, RG), todos não confiáveis (`INVALIDO` e `VALIDACAO_INDISPONIVEL`) e `trusted_identifiers=0`. Controle negativo com falha conhecida — CPF de DV válido injetado no corpus — reprovou o gate com `SYNTHETIC_IDENTIFIER_ACCEPTED`, confirmando que o critério não é decorativo. Testes: 85 verdes em `backend/ocr_tests` (41 novos), incluindo conferência do DV de CPF contra implementação de referência independente e mutação de cada dígito dos vetores válidos.
...
- **Defeito real que a allowlist escondia:** com `REGRAS_SISTEMICAS_CONHECIDAS` vazia, o CI expôs `aria-required-children` em `/analytics` — o `<v-list>` de "Destinos analíticos" afirmava `role="list"` tendo filhos `role="button"` (que é o papel correto para eles: são botões de navegação com `tabindex` e Enter). Corrigido com `role="presentation"` no container, igual ao menu lateral. Uma varredura dos blocos `<v-list>` das 55 telas confirma que só `AppLayout` e `AnalyticsHubView` tinham papel de filho incompatível.
