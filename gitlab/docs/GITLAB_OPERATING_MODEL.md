...
O projeto GitLab **existe e está provisionado** (remote `gitlab` configurado, `git@gitlab.com:ericson-j-santos/reqsys-v2-enterprise-real.git`). A branch `main` do GitLab está **em dia** com o GitHub `main` (confirmado via `git ls-remote gitlab main` em 2026-08-25, mesmo commit do `origin`/local) — a sincronização deixou de ser um evento manual pontual e passou a acompanhar o GitHub de perto. Segue sem evidência de que uma pipeline tenha rodado de fato em runner GitLab real — checar isso exige acesso ao painel CI/CD do projeto (`gitlab.com/ericson-j-santos/reqsys-v2-enterprise-real/-/pipelines`), que não foi possível verificar remotamente nesta sessão. O texto abaixo descreve o que roda *se* executado, não o que já foi validado ao vivo.
