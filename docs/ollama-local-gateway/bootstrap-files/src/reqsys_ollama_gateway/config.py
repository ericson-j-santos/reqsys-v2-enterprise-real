...
def load_settings() -> Settings:
    ...
    origins = tuple(
        origin.strip()
        for origin in os.getenv('REQSYS_ALLOWED_ORIGINS', 'http://localhost:3000,http://localhost:5173').split(',')
        if origin.strip()
    )
    settings = Settings(
        env=env,
        ...
        ollama_timeout_seconds=int(os.getenv('REQSYS_OLLAMA_TIMEOUT_SECONDS', '120')),
    )
