...
- MCP health identificado: `127.0.0.1:8011/health` (somente loopback, nunca publicado pelo Funnel)
...
O bridge MCP consome o bearer pré-provisionado em `OLLAMA_MCP_BEARER_TOKEN`; o valor não é gravado em metadata, estado, log ou evidência. Se o bearer estiver ausente, o supervisor falha fechado com `mcp_bearer_token_not_configured`. O subprocesso recebe uma allowlist mínima de variáveis do SO/Python e não herda `GITHUB_TOKEN` nem variáveis arbitrárias do runner. O bridge continua chamando exclusivamente o gateway loopback `:8008`.
...
## Pós-reboot
Após um reboot real, executar `postboot-check --require-reboot`. A prova só é válida quando o boot epoch mudou, a persistência é `AtStartup`, Ollama/gateway/bridge MCP/backend estão saudáveis, o health MCP identificado em `:8011` confirma `auth_configured=true` e `secret_exposed=false`, e existe smoke posterior.
