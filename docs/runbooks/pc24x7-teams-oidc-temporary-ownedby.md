...
A conta administrativa que concede/remove a appRoleAssignment precisa das permissões/role administrativas necessárias para app-role assignment. Isso não é transferido ao CI e não vira uma credencial persistente de automação.

## Evidência mínima

A execução só termina `ready` quando comprova tenant correto, ownership, ausência de `OwnedBy` preexistente, grant temporário criado e relido, workflow verde no SHA exato da `main`, FIC com issuer/subject/audience exatos, grant removido e releitura final sem a permissão temporária. A saída mantém `application_readwrite_all_granted=false` e `secret_value_exposed=false`.
