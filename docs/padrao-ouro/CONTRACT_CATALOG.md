...
| `POST /tokens` | JWT admin | `{ label, key_patterns[] }` | `{ id, label, token, key_patterns, aviso }` (token em claro só nesta resposta) |
| `GET /tokens` | JWT admin | — | `{ tokens: [{ id, label, key_patterns, criado_em, ultimo_uso_em, revogado }] }` |
| `DELETE /tokens/{id}` | JWT admin | — | `{ id, revogado }` |
| `GET /segredos/{key}` | `X-Vault-Token` (escopado ou legado) | — | `{ key, value }` |
| `POST /resolver` | `X-Vault-Token` (escopado ou legado) | `{ key }` | `{ key, value }` |
...
Tokens escopados: `X-Vault-Token` é validado primeiro contra `VaultToken` (hash sha256, `key_patterns` glob) — a chave pedida precisa casar um padrão ou a rota retorna `403`. Só cai no `VAULT_API_TOKEN` global (sem restrição) se nenhum token escopado casar; esse uso legado gera `AuditoriaEvento` `COFRE_TOKEN_LEGADO_USADO`. Toda gravação/remoção/leitura gera auditoria (ADR-003) sem expor o valor do segredo (ADR-002). Rate limit dedicado em `/api/v1/cofre/` (nginx, zona `cofre`).
