const SQL_KEYWORDS = new Set([
  ...
  'on', 'group', 'by', 'order', 'having', 'limit', 'offset', 'with', 'recursive',
  'union', 'all', 'distinct', 'case', 'when', 'then', 'else', 'end', 'as', 'and', 'or'
])
...
function extractClause(sql, startPattern, endPatterns) {
  const normalized = normalizeSql(sql)
  const end = endPatterns.join('|')
  const regex = new RegExp(`${startPattern}\\s+(.+?)(?=\\s+(?:${end})\\b|$)`, 'i')
...
function calculateRisk({ sql, columns, tables, joins, filters, ctes }) {
  ...
  if (/over\s*\(/i.test(sql)) {
    findings.push({ severity: 'info', type: 'analytics', message: 'Função de janela detectada. Validar partição e ordenação da métrica.' })
  }
...
function summarize({ tables, joins, filters, groupBy, orderBy, ctes }) {
  ...
  const origem = tables.map((item) => item.table).join(', ')
  const partes = []

  if (ctes.length) partes.push(`usa ${ctes.length} CTE(s) como etapa(s) intermediária(s)`)
  if (origem) partes.push(`consulta dados de ${origem}`)
