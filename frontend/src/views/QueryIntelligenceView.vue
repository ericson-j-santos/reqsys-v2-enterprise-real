  ...
  <section class="query-page">
    <div class="query-header">
      <div>
        ...
        <p class="muted">
          Analise consultas SQL sem execução, extraia intenção lógica, riscos, lineage inicial e grafo navegável.
        </p>
      </div>
      <v-chip :color="riskColor" variant="tonal" size="large">
        Risco {{ analysis.riskScore }} · {{ analysis.riskLevel }}
      </v-chip>
    </div>
      ...
      <v-col cols="12" md="7">
        <v-card class="panel" elevation="0">
          ...
          <v-card-text>
            ...
            />
            <div class="actions">
              <v-btn color="primary" prepend-icon="mdi-database-search" @click="analisar">
                Analisar
              </v-btn>
              <v-btn variant="tonal" prepend-icon="mdi-refresh" @click="carregarExemplo">
                Exemplo avançado
              </v-btn>
            </div>
          </v-card-text>
        ...
        <v-card class="panel fill" elevation="0">
          ...
          <v-card-text>
            ...
            <div class="metric-grid">
              <div class="metric">
                <span>Tabelas</span>
                <strong>{{ analysis.tables.length }}</strong>
              </div>
              <div class="metric">
                <span>Joins</span>
              ...
              <div class="metric">
                <span>CTEs</span>
                <strong>{{ analysis.ctes.length }}</strong>
              </div>
              <div class="metric">
                <span>Achados</span>
        ...
        <v-card class="panel" elevation="0">
          ...
          <v-card-text>
            <div class="graph" role="list" aria-label="Grafo lógico da consulta">
              <div
                v-for="node in analysis.graph.nodes"
                :key="node.id"
                class="graph-node"
        ...
        <v-card class="panel" elevation="0">
          ...
          <v-card-text>
            ...
            <v-alert v-if="!analysis.findings.length" type="success" variant="tonal">
              Nenhum achado crítico identificado na análise estática inicial.
            </v-alert>
          </v-card-text>
        </v-card>
      </v-col>
      ...
      <v-card-text>
        <v-table density="compact">
          ...
          <tbody>
            <tr>
              <td>Colunas</td>
              <td>{{ analysis.columns.join(', ') || 'Não detectado' }}</td>
            </tr>
            <tr>
              <td>Tabelas</td>
              <td>{{ tableLabels || 'Não detectado' }}</td>
            </tr>
            <tr>
              <td>Filtros</td>
              <td>{{ analysis.filters || 'Não detectado' }}</td>
            </tr>
            <tr>
              <td>Agrupamento</td>
              <td>{{ analysis.groupBy || 'Não detectado' }}</td>
            </tr>
            <tr>
              <td>Ordenação</td>
              <td>{{ analysis.orderBy || 'Não detectado' }}</td>
            </tr>
          </tbody>
        </v-table>
      </v-card-text>
...
const exemploAvancado = `WITH vendas_mes AS (
    ...
    SELECT
        ...
        mes,
        total_mes,
        RANK() OVER (PARTITION BY mes ORDER BY total_mes DESC) AS posicao
    FROM vendas_mes
)
SELECT
    cliente_id,
...
function carregarExemplo() {
  sql.value = exemploAvancado
  analisar()
}

const tableLabels = computed(() => analysis.value.tables.map((item) => (
  item.alias ? `${item.table} (${item.alias})` : item.table
)).join(', '))

const riskColor = computed(() => {
  const level = analysis.value.riskLevel
  if (level === 'critical') return 'red'
  if (level === 'high') return 'deep-orange'
