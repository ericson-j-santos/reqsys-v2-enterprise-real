...
def parse_coverage_percent(output: str) -> float | None:
    ...
    match = re.search(r"total.*?(\d+)%", output, re.IGNORECASE)
    if match:
        return float(match.group(1))
    return None
...
def run_mutation_dimension() -> DimensionResult:
    """Probe leve de mutation testing em rotas criticas via pytest direcionado."""
    started = datetime.now(timezone.utc)
    targets = [
        "tests/test_public_smoke_routes.py",
        "tests/test_runtime_evidence_ingestion_contract.py",
        "tests/test_runtime_evidence_analytics.py",
    ]
    ...
    if not existing:
        return DimensionResult(
            ...
            summary="nenhum alvo de mutation probe encontrado",
            details={"targets": targets},
            blockers=["targets ausentes"],
            recommendations=["Garantir testes de contrato em rotas criticas"],
        )
    ...
    score = min(100.0, killed_mutations * 10) if passed else max(0.0, killed_mutations * 5)
    return DimensionResult(
        dimension="mutation",
        status="passed" if passed else "failed",
        score=round(score, 2),
        duration_seconds=round(duration, 2),
        summary=f"mutation probe em {len(existing)} modulos criticos",
        details={
        ...
        blockers=[f"{total_failures} falha(s) no probe"] if total_failures else [],
        recommendations=[]
        if passed
        else ["Reforcar testes de regressao em rotas publicas e contratos runtime"],
    )
...
def run_contract_dimension() -> DimensionResult:
    ...
    all_tests = [str(path.relative_to(ROOT)) for path in contract_tests] + [
        str(path.relative_to(ROOT)) for path in repo_contract_tests
    ]

    if contract_tests:
        rel_tests = [str(path.relative_to(BACKEND)) for path in contract_tests]
        proc = run_command(
            [sys.executable, "-m", "pytest", *rel_tests, "-q", "--tb=line"],
            cwd=BACKEND,
            env=backend_test_env(),
        )
        summary = parse_pytest_summary(proc.stdout + proc.stderr)
    ...
    if validator.exists():
        ...
        details["checks"].append(
            {
                "name": "artifact_contract_validator",
        ...
        blockers.append("artifact_contract_validator.py ausente")
    ...
    status = "failed" if hard_failures or any("falharam" in b or "ausente" in b for b in blockers[:2]) else (
        "warning" if recommendations else "passed"
    )
    ...
    return DimensionResult(
        ...
        score=score,
        duration_seconds=round(duration, 2),
        summary=f"{len(all_tests)} contract test file(s), {len(details['checks'])} check(s)",
        details={**details, "contract_test_files": all_tests},
        blockers=blockers,
...
def run_schema_dimension() -> DimensionResult:
    ...
    if governance_script.exists():
        ...
        details["schema_governance_gate"] = {
            "status": "passed" if proc.returncode == 0 else "warning",
            "exit_code": proc.returncode,
        }
        if proc.returncode != 0:
    ...
    return DimensionResult(
        ...
        score=score,
        duration_seconds=round(duration, 2),
        summary=f"{valid}/{len(schema_files)} schemas JSON validos",
        details=details,
        blockers=blockers,
        recommendations=[] if status == "passed" else ["Corrigir schemas invalidos em docs/contracts/"],
    )
...
def run_ci_watch_dimension() -> DimensionResult:
    ...
    if pr_watch.exists():
        ...
        details["checks"].append(
            {"name": "pr_ci_watch_compile", "status": "passed" if proc.returncode == 0 else "failed"}
        )
        if proc.returncode != 0:
    ...
    return DimensionResult(
        dimension="ci-watch",
        status=status,
        score=100.0 if status == "passed" else max(0.0, 100.0 - len(blockers) * 20),
        duration_seconds=round(duration, 2),
        summary="estrutura local do PR CI Watch validada" if status == "passed" else f"{len(blockers)} gap(s)",
        details=details,
        blockers=blockers,
        recommendations=[] if status == "passed" else ["Restaurar guardrails do PR CI Watch"],
    )
...
def consolidate(dimension_reports: list[dict[str, Any]], *, repository: str, run_id: str) -> dict[str, Any]:
    ...
    decision_map = {
        "passed": "continuar_incremento_qualidade",
        "warning": "validar_warnings_antes_de_merge",
...
def _recommended_actions(global_state: str, dimensions: list[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    if global_state == "failed":
        actions.append(
            {
                "priority": "P0",
                "action": "corrigir_dimensoes_falhas",
                "detail": "Trilha D com falha — revisar blockers por dimensao",
            }
        )
    for item in dimensions:
        if item.get("status") == "failed":
            actions.append(
                {
                    "priority": "P1",
                    "action": f"corrigir_{item['dimension']}",
    ...
    if global_state == "passed":
        actions.append(
            {
                "priority": "P3",
                "action": "manter_gates_ci_obrigatorio",
                "detail": "Trilha D verde — manter CI ReqSys v2 Enterprise como gate canonico",
            }
        )
    return actions
...
def write_consolidated_report(report: dict[str, Any], output_dir: Path) -> tuple[Path, Path]:
    ...
    lines = [
        ...
        f"- Dimensões: {report['dimensions_total']}",
        f"- Paralelizável: {report['parallelizable']}",
        "",
        "## Dimensões",
        "",
        "| Dimensão | Status | Score | Resumo |",
        "|---|---|---:|---|",
    ]
    for item in report["dimensions"]:
        lines.append(
            f"| {item['dimension']} | {item['status']} | {item['score']} | {item['summary']} |"
        )
    if report["blockers"]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default="all",
        help="Dimensão a executar (CI matrix usa uma por job)",
    )
    parser.add_argument(
        "--consolidate",
        action="store_true",
        help="Consolidar JSONs de dimensões já gerados no output-dir",
    )
    ...
    parser.add_argument(
        "--fail-on-error",
        action="store_true",
        help="Retorna exit code 1 quando estado consolidado for failed",
    )
