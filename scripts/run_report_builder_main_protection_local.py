#!/usr/bin/env python3
"""Protect report-builder-platform/main using only Noteri's local GitHub authentication."""
...
def _required_check_states(checks: list[Any]) -> dict[str, bool]:
...
def protection_compliant(payload: dict[str, Any]) -> bool:
    ...
    return (
        required_names.issubset(contexts | checks)
...
def _write_evidence(path: Path, payload: dict[str, Any]) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(
        prefix="report-builder-protection-",
        suffix=".json",
...
def _blocked(
    ...
    check_states: dict[str, bool] | None = None,
) -> int:
    _write_evidence(
        path,
        {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Protect report-builder-platform/main through Noteri local GitHub auth"
    )
    ...
    try:
        ...
        return _blocked(output, reason, target_sha, check_states)
