#!/usr/bin/env python3
"""Install a temporary exact Owner Risk3 action for report-builder-platform/main protection."""
...
ACTION_ID = "reqsys.report-builder-main-protection.dev"
SCOPE = "repo://ericson-j-santos/report-builder-platform/branch/main"
MANAGED_BY = "reqsys-report-builder-main-protection-once"
ENABLE_CONFIRM = "ENABLE-REPORT-BUILDER-MAIN-PROTECTION-ONCE"
DISABLE_CONFIRM = "DISABLE-REPORT-BUILDER-MAIN-PROTECTION-ONCE"
EVIDENCE_PATH = "artifacts/report-builder-main-protection-noteri/evidence.json"
COMMAND = [
    "python",
    "scripts/run_report_builder_main_protection_local.py",
    "--output",
    EVIDENCE_PATH,
]
...
def default_config_path() -> Path:
    ...
    if local:
        return (
            ...
            / "ReqSys"
            / "CommandGateway"
            / "owner-risk3-exceptions.local.json"
        )
    return (
        Path.home()
        / ".reqsys-command-gateway"
        / "owner-risk3-exceptions.local.json"
    )
...
def enable(path: Path, ttl_minutes: int) -> dict:
    ...
    if isinstance(existing, dict) and existing.get("managed_by") != MANAGED_BY:
        ...
        return {
            "status": "existing_exact_action_reused",
            "action_id": ACTION_ID,
            "managed": False,
        }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Authorize one Report Builder main protection action"
    )
