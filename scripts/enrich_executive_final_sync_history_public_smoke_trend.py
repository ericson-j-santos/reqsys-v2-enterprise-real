#!/usr/bin/env python3
"""Consolida a tendência temporal do smoke público final no Estado Único.

O resultado é estritamente informativo e não altera readiness, deploy,
production_ready ou decisões de promoção.
"""
...
OK_STATUS = "PUBLIC_FINAL_HISTORY_SMOKE_OK"
...
def _environment_summary(history: dict[str, Any], environment: str) -> dict[str, Any]:
    samples = [
        item for item in history.get("samples", [])
        if isinstance(item, dict) and item.get("environment") == environment
    ]
...
def build_state(histories: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    minimum_stable_sequence = min(
        (item["stable_sequence"] for item in environments.values()),
        default=0,
    )
    fingerprints = {
        item["fingerprint"] for item in environments.values() if item["fingerprint"]
    }
    coverage_complete = not missing
    synchronized = coverage_complete and all(item["synchronized"] for item in environments.values())
    common_fingerprint = next(iter(fingerprints)) if len(fingerprints) == 1 else ""
    no_drift = coverage_complete and synchronized and bool(common_fingerprint)
    eligible = no_drift and all(item["evidence_sufficient"] for item in environments.values())

    if not coverage_complete:
        status = "insufficient-environment-coverage"
    elif any(item["latest_status"] != OK_STATUS for item in environments.values()):
        status = "attention"
    elif not synchronized or not common_fingerprint:
    ...
    return {
        ...
        "weighted_pass_rate_percent": round(weighted_pass_rate, 2),
        "minimum_stable_sequence": minimum_stable_sequence,
...
def enrich(runtime_index: dict[str, Any], executive_brief: dict[str, Any], histories: dict[str, dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    brief[CARD_KEY] = {
        ...
        "common_fingerprint": state["common_fingerprint"],
