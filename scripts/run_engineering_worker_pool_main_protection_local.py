#!/usr/bin/env python3
"""Protege engineering-worker-pool/main via autenticação GitHub local de hosts governados."""
...
TARGET_REPOSITORY = "ericson-j-santos/engineering-worker-pool"
TARGET_BRANCH = "main"
REQUIRED_CHECKS = ("test",)
API_VERSION = "2026-03-10"
ALLOWED_HOSTS = frozenset({"DESKTOP-PDQK954", "NOTERI"})
...
def _repository() -> dict[str, Any]:
    return _gh_json(
        [
            ...
            "-H", "Accept: application/vnd.github+json",
            "-H", f"X-GitHub-Api-Version: {API_VERSION}",
            f"repos/{TARGET_REPOSITORY}",
        ],
        "target_repository_read_failed",
    )
...
def _write_evidence(path: Path, payload: dict[str, Any]) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(prefix="worker-pool-protection-", suffix=".json", dir=str(target.parent))
    try:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Protege engineering-worker-pool/main via auth local do PC24x7")
    ...
    try:
        ...
        repo_before = _repository()
        if before.get("protected") is True:
            current_protection = _protection()
            if protection_compliant(current_protection) and repo_before.get("allow_auto_merge") is True:
        ...
        update = _gh(
            ...
            stdin=json.dumps(protection_payload, separators=(",", ":")),
        )
        ...
        repo_update = _gh(
            ...
            stdin=json.dumps({"allow_auto_merge": True}, separators=(",", ":")),
        )
        if repo_update.returncode != 0:
            raise ProtectionError("auto_merge_update_failed")
        ...
        after_repo = _repository()
        if after_repo.get("allow_auto_merge") is not True:
            raise ProtectionError("auto_merge_readback_mismatch")
