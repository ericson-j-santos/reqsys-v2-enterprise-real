#!/usr/bin/env python3
"""Injeta a tendência do smoke público do índice executivo no Ops Dashboard."""
...
CARD_ID = "executive-sync-stability-index-public-smoke-trend"
HOOK = "<!-- REQSYS_EXECUTIVE_SYNC_STABILITY_INDEX_PUBLIC_SMOKE_TREND -->"


def _safe_contract(payload: dict[str, Any]) -> dict[str, Any]:
    source = payload.get("cards", {}).get(CARD_KEY, {})
    environments = source.get("environments", {}) if isinstance(source, dict) else {}
    return {
        "trend": str(source.get("trend", "insufficient-environment-coverage")),
...
def _render(contract: dict[str, Any]) -> str:
  ...
  <h2>Tendência pública do índice executivo</h2>
  <dl>
    <dt>Tendência</dt><dd>{html.escape(contract["trend"])}</dd>
...
def main() -> int:
    ...
    args.runtime_index.write_text(json.dumps(updated_runtime, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    return 0
