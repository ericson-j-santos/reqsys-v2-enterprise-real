#!/usr/bin/env python3
"""Protege reqsys-data-platform/main usando autenticacao GitHub local do Noteri."""
...
TARGET_REPOSITORY = "ericson-j-santos/reqsys-data-platform"
TARGET_BRANCH = "main"
EXPECTED_TARGET_SHA = "721a1c57c21b0732bfaa1fac62a8a1af531fe6d0"
REQUIRED_CHECKS = (
    "repository-hygiene",
    "backup-restore-e2e",
    "sqlite-postgres-e2e",
...
def _required_checks_green(sha: str) -> bool:
    payload = _check_runs(sha)
    ...
    return set(REQUIRED_CHECKS).issubset(green)
...
def _write_evidence(path: Path, payload: dict[str, Any]) -> None:
    ...
    fd, temp_name = tempfile.mkstemp(prefix="data-protection-", suffix=".json", dir=str(target.parent))
    try:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Protege reqsys-data-platform/main via auth local do Noteri")
    ...
    try:
        ...
        if not _required_checks_green(target_sha):
        ...
        if current_sha != EXPECTED_TARGET_SHA:
        ...
        if after_sha != EXPECTED_TARGET_SHA:
