...
def render_markdown(pr: PullRequestContext, files: list[ChangedFile], severity: str, score: int, findings: list[QualityFinding]) -> str:
    ...
    emoji = {"ok": "🟢", "warning": "🟡", "critical": "🔴"}.get(severity, "⚪")
    decision = {
...
def main() -> int:
    ...
    payload = {
        ...
        "summary": {
            ...
            "tests": sum(1 for file in files if file.is_test),
            "docs": sum(1 for file in files if file.is_doc),
            "workflows": sum(1 for file in files if file.is_workflow),
            "sensitive": sum(1 for file in files if file.is_sensitive),
        },
