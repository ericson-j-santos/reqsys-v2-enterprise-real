#!/usr/bin/env python3
"""Configura uma exceção Owner Risk3 exata e temporária para restaurar auth do Worker Pool DEV."""
...
ACTION_ID = "reqsys.worker-pool-auth-file-restore.dev"
SCOPE = "repo://reqsys/environment/dev/worker-pool"
MANAGED_BY = "reqsys-worker-pool-auth-restore-once"
ENABLE_CONFIRM = "ENABLE-WORKER-POOL-AUTH-RESTORE-ONCE"
DISABLE_CONFIRM = "DISABLE-WORKER-POOL-AUTH-RESTORE-ONCE"
EVIDENCE_PATH = "artifacts/codex-worker-pool-auth-restore-dev/evidence.json"
COMMAND = [
    "python",
    "scripts/run_worker_pool_auth_restore_dev.py",
    "--output",
    EVIDENCE_PATH,
]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Autoriza uma única restauração Risk3 do Worker Pool DEV")
