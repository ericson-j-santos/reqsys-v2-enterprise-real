#!/usr/bin/env python3
"""Orquestra o ciclo temporário Application.ReadWrite.OwnedBy para a FIC DEV.

Requer sessões Microsoft Entra e GitHub já autenticadas localmente. O script não
recebe senha, MFA, token, client secret ou segredo por argumento.

A automação NUNCA concede Application.ReadWrite.All automaticamente.
"""
...
WORKFLOW_FILE = "pc24x7-teams-oidc-environment-bootstrap.yml"
...
def _tool(name: str) -> str:
    for candidate in (name, f"{name}.exe", f"{name}.cmd", f"{name}.bat"):
        ...
        if resolved:
            return resolved
    raise BootstrapError(f"Ferramenta obrigatória não encontrada: {name}")


def _run(tool: str, args: list[str], *, check: bool = True) -> subprocess.CompletedProcess[str]:
    result = subprocess.run([_tool(tool), *args], check=False, capture_output=True, text=True, encoding="utf-8")
...
def _az_json(args: list[str]) -> Any:
    try:
        ...
        raise BootstrapError("Azure CLI/Microsoft Graph retornou JSON inválido.") from exc
...
def _resolve_context(mutator_client_id: str, expected_tenant: str) -> Context:
    ...
    if not tenant:
        raise BootstrapError("Sessão Microsoft Entra/Azure ausente ou inválida.")
    ...
    if not all((app_id, sp_id, graph_sp)):
        raise BootstrapError("Não foi possível resolver aplicação/service principals.")
...
def _graph(method: str, url: str, *, body: dict[str, Any] | None = None) -> Any:
    ...
    return _az_json(args) if method != "DELETE" else _run("az", args)
...
def _existing_ownedby_assignment(sp_id: str, graph_sp_id: str) -> dict[str, Any] | None:
    return next((item for item in _assignments(sp_id) if str(item.get("resourceId")) == graph_sp_id and str(item.get("appRoleId")) == OWNED_BY_APP_ROLE_ID), None)


def _grant_ownedby(sp_id: str, graph_sp_id: str) -> str:
    payload = _graph("POST", f"https://graph.microsoft.com/v1.0/servicePrincipals/{sp_id}/appRoleAssignments", body={"principalId": sp_id, "resourceId": graph_sp_id, "appRoleId": OWNED_BY_APP_ROLE_ID})
...
def _revoke_assignment(sp_id: str, assignment_id: str) -> None:
    _graph("DELETE", f"https://graph.microsoft.com/v1.0/servicePrincipals/{sp_id}/appRoleAssignments/{assignment_id}")
...
def _dispatch_bootstrap_workflow(repository: str, expected_sha: str) -> dict[str, Any]:
    ...
    for _ in range(24):
        ...
        if run:
            break
    if not run:
        raise BootstrapError("Run do workflow não foi localizado no SHA esperado.")
    run_id = int(run["databaseId"])
    _run("gh", ["run", "watch", str(run_id), "--repo", repository, "--exit-status"])
    final = json.loads((_run("gh", ["run", "view", str(run_id), "--repo", repository, "--json", "databaseId,headSha,status,conclusion,url"]).stdout or "{}"))
    if final.get("headSha") != expected_sha or final.get("conclusion") != "success":
        raise BootstrapError("Workflow OIDC não concluiu verde no SHA esperado.")
    return {"run_id": run_id, "head_sha": expected_sha, "conclusion": "success", "url": final.get("url")}


def _verify_fic(app_object_id: str, credential_name: str, subject: str) -> bool:
    payload = _graph("GET", f"https://graph.microsoft.com/v1.0/applications/{app_object_id}/federatedIdentityCredentials")
    values = payload.get("value", []) if isinstance(payload, dict) else []
    return any(isinstance(item, dict) and str(item.get("name") or "") == credential_name and str(item.get("issuer") or "") == ISSUER and str(item.get("subject") or "") == subject and item.get("audiences") == [AUDIENCE] for item in values)
...
def execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if ctx.mutator_service_principal_id not in _owners(ctx.mutator_app_object_id):
        return {"status": "blocked", "reason": "MUTATOR_NOT_OWNER", "requires_human_decision": "assign_mutator_service_principal_as_owner", "environment": "dev", "application_readwrite_all_granted": False, "secret_value_exposed": False}
    if _existing_ownedby_assignment(ctx.mutator_service_principal_id, ctx.graph_service_principal_id):
        return {"status": "blocked", "reason": "OWNEDBY_ALREADY_PRESENT", "requires_human_decision": "preexisting_permission_must_not_be_revoked_implicitly", "environment": "dev", "application_readwrite_all_granted": False, "secret_value_exposed": False}
    if args.dry_run:
        return {"status": "dry_run", "environment": "dev", "planned_action": "grant_ownedby_dispatch_fic_revoke_ownedby", "application_readwrite_all_granted": False, "secret_value_exposed": False}
    ...
    try:
        ...
        if not _existing_ownedby_assignment(ctx.mutator_service_principal_id, ctx.graph_service_principal_id):
            raise BootstrapError("Releitura não comprovou OwnedBy temporário.")
        main_sha = _main_sha(args.repository)
        if not main_sha:
            raise BootstrapError("Não foi possível capturar o SHA atual da main.")
        workflow_evidence = _dispatch_bootstrap_workflow(args.repository, main_sha)
        if not _verify_fic(ctx.mutator_app_object_id, args.credential_name, subject):
            raise BootstrapError("Releitura administrativa não comprovou a FIC esperada.")
    finally:
        if assignment_id:
    ...
    return {"status": "ready", "environment": "dev", "subject": subject, "credential_name": args.credential_name, "ownedby_temporarily_granted": True, "ownedby_revoked": True, "application_readwrite_all_granted": False, "rbac_changed": False, "secret_value_exposed": False, "workflow": workflow_evidence}


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ciclo temporário OwnedBy para FIC PC24x7 Teams DEV")
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(json.dumps({"status": "blocked", "reason": str(exc), "environment": "dev", "application_readwrite_all_granted": False, "secret_value_exposed": False}, ensure_ascii=False, sort_keys=True))
        return 4
