...
Mantém o contrato existente: os painéis continuam consumindo apenas
`runtime-executive-index.json`. O script é idempotente e offline.
"""
...
OPS_FUNCTION = """
    function renderExecutiveReadinessVisual(payload) {
      ...
      document.getElementById('exec-readiness-production-ready').innerHTML = productionReady
        ? '<span class="status passed">SIM</span>'
        : '<span class="status blocked">NÃO</span>';
      ...
      document.getElementById('exec-readiness-details-rows').innerHTML = rows.map(([label, value]) => `<tr><td>${label}</td><td>${value}</td></tr>`).join('');
