...
def fluxo_excel_para_planner() -> dict[str, Any]:
    list_commands = _excel(
        'GetItems',
        {**_excel_base('tbAtualizacoesPlanner'), '$filter': "AtualizarPlanner eq 'SIM'"},
    )
        ...
        'PatchItem',
        {
            **_excel_base('tbAtualizacoesPlanner'),
            ...
            'item': {
                ...
                'DetalheConflito': 'Planner mudou desde a ultima leitura; nenhuma escrita foi aplicada.',
                'CorrelationId': "@workflow()?['run']?['name']",
            },
        },
    )
    conflict_memory = _excel(
        'PatchItem',
        {
            **_excel_base('tbMemoriaCopilot'),
            'idColumn': 'MemoryId',
            'id': "@items('Para_cada_comando')?['MemoryId']",
            'item': {'PlannerSyncStatus': 'CONFLITO', 'CorrelationId': "@workflow()?['run']?['name']", 'AtualizadoEm': '@utcNow()'},
        },
    ...
    conflict_history = _excel(
        'AddRowV2',
        {
            **_excel_base('tbHistoricoCopilot'),
            'item': {
                'EventId': '@guid()', 'MemoryId': "@items('Para_cada_comando')?['MemoryId']",
                'PlannerTaskId': "@items('Para_cada_comando')?['PlannerTaskId']", 'Versao': '',
                'Origem': 'excel', 'TipoEvento': 'CONFLITO',
                'Resumo': 'Alteracao Excel bloqueada porque Planner mudou',
                'PlannerSignature': "@outputs('Assinatura_atual')",
                'CorrelationId': "@workflow()?['run']?['name']", 'CriadoEm': '@utcNow()',
            },
        },
    ...
    update_task = _planner(
        'UpdateTask_V3',
        {
            ...
            'percentComplete': "@int(coalesce(items('Para_cada_comando')?['PlannerPercentual'],0))",
            'dueDateTime': "@if(empty(items('Para_cada_comando')?['PlannerPrazo']),null,items('Para_cada_comando')?['PlannerPrazo'])",
        },
    )
    finish_command = _excel(
        'PatchItem',
        {
            **_excel_base('tbAtualizacoesPlanner'), 'idColumn': 'MemoryId',
            'id': "@items('Para_cada_comando')?['MemoryId']",
            'item': {'AtualizarPlanner': 'NAO', 'ResultadoSync': 'SINCRONIZADO', 'DetalheConflito': '', 'CorrelationId': "@workflow()?['run']?['name']"},
        },
    ...
    finish_memory = _excel(
        'PatchItem',
        {
            **_excel_base('tbMemoriaCopilot'), 'idColumn': 'MemoryId',
            'id': "@items('Para_cada_comando')?['MemoryId']",
            'item': {
                ...
                'CorrelationId': "@workflow()?['run']?['name']", 'AtualizadoEm': '@utcNow()',
            },
        },
        ...
        'AddRowV2',
        {
            **_excel_base('tbHistoricoCopilot'),
            'item': {
                'EventId': '@guid()', 'MemoryId': "@items('Para_cada_comando')?['MemoryId']",
                'PlannerTaskId': "@items('Para_cada_comando')?['PlannerTaskId']", 'Versao': '',
                'Origem': 'excel', 'TipoEvento': 'PLANNER_ATUALIZADO', 'Resumo': 'Alteracao autorizada aplicada ao Planner',
                'PlannerSignature': "@outputs('Assinatura_desejada')",
                'CorrelationId': "@workflow()?['run']?['name']", 'CriadoEm': '@utcNow()',
            },
        },
        run_after={'Atualizar_memoria': ['Succeeded']},
    ...
    decision = {
        ...
        'actions': {
            'Marcar_comando_conflito': conflict_command,
            'Marcar_memoria_conflito': conflict_memory,
            'Historico_conflito': conflict_history,
        },
        'else': {
            'actions': {
                'Atualizar_Planner': update_task,
                'Finalizar_comando': finish_command,
                'Atualizar_memoria': finish_memory,
                'Historico_sucesso': finish_history,
            }
        },
    ...
    foreach = {
        ...
        'actions': {
            'Reler_tarefa': get_task,
            'Assinatura_atual': current_signature,
            'Assinatura_desejada': desired_signature,
            'Aplicar_somente_sem_conflito': decision,
        },
...
def fluxo_saude() -> dict[str, Any]:
    ...
    filter_errors = {
        ...
        'inputs': {
            ...
            'where': "@or(equals(toUpper(coalesce(item()?['PlannerSyncStatus'],'')),'CONFLITO'),equals(toUpper(coalesce(item()?['PlannerSyncStatus'],'')),'ERRO'))",
        },
    ...
    decision = {
        ...
        'actions': {
            'Falhar_visivelmente': {
                ...
                'inputs': {
                    'runStatus': 'Failed',
                    'runError': {'code': 'COPILOT_MEMORY_HEALTH', 'message': "@concat('Conflitos/erros encontrados: ',string(length(body('Filtrar_problemas'))))"},
                },
                'runAfter': {},
            }
        },
