...
Este módulo é carregado por `app.api.__init__` antes do registro dos routers no
FastAPI. Ele adiciona uma rota com o mesmo contrato do endpoint legado de
transição, posicionada antes da rota original em `requisitos.api_router`.

A abordagem preserva compatibilidade e reduz risco: se este módulo não for
carregado, o endpoint legado continua existindo; quando carregado, a transição
passa a publicar o evento governado `REQUISITO_TRANSICIONADO` no Runtime Core.
"""
...
def transicionar_requisito_runtime_core(
    ...
    O retorno mantém os campos existentes (`schema_version`, `requisito`,
    `workflow`, `transicao`) e adiciona `runtime_event` como evidência técnica
    opcional e compatível para clientes que quiserem consumi-la.
    """
