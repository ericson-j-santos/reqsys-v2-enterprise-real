...
async def analisar_documento_demanda(
    ...
    if existente:
        return ok({'id': existente.id, 'demanda_ref': existente.demanda_ref, 'sha256': existente.sha256, 'status': existente.status, 'candidatos': json.loads(existente.candidatos_json or '[]'), 'idempotente': True}, existente.correlation_id)
    ...
    registro = DocumentoDemandaAnalise(
        demanda_ref=demanda_ref.strip(), nome_arquivo=arquivo.filename or 'arquivo',
        content_type=content_type, sha256=sha256, correlation_id=correlation_id,
        status=status, texto_extraido=texto, candidatos_json=serializar_candidatos(candidatos),
    )
    db.add(registro)
    try:
        db.commit()
    ...
    return ok({'id': registro.id, 'demanda_ref': registro.demanda_ref, 'sha256': registro.sha256, 'status': registro.status, 'candidatos': [c.__dict__ for c in candidatos], 'idempotente': False, 'incorporacao_automatica': False}, correlation_id)


@router.get('/{analise_id}')
def obter_analise_documento(analise_id: int, db: Session = Depends(get_db), user: dict = Depends(require_admin)):
    del user
    ...
    return ok({'id': registro.id, 'demanda_ref': registro.demanda_ref, 'nome_arquivo': registro.nome_arquivo, 'content_type': registro.content_type, 'sha256': registro.sha256, 'status': registro.status, 'candidatos': json.loads(registro.candidatos_json or '[]'), 'incorporacao_automatica': False}, registro.correlation_id)
