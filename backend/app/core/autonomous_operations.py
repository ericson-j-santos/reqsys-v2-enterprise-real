...
def criar_indicadores_base() -> list[IndicadorMaturidade]:
    return [
        IndicadorMaturidade(
            ...
            evidencias=['Monitoramento operacional existe; auto-remediação real ainda não executa ações destrutivas.'],
            gaps=['Auto-remediação real pendente.', 'Self-healing pendente.', 'Orquestração de recuperação inicial.'],
            proximas_acoes=['Persistir incidentes e decisões de remediação.', 'Conectar métricas reais de runtime.'],
        ),
        IndicadorMaturidade(
            ...
            evidencias=['Snapshot operacional versionado expõe correlation_id.'],
            gaps=['OpenTelemetry end-to-end pendente.', 'Alertas inteligentes pendentes.'],
        ...
        IndicadorMaturidade(
            ...
            proximas_acoes=['Versionar policies de remediação.', 'Bloquear ações autônomas sem auditoria.'],
        ),
        IndicadorMaturidade(
            ...
            tendencia_evidenciada='consolidacao_continua',
            impacto_operacional='alta',
            evidencias=['Workflows e testes existem; estabilidade exige CI verde recorrente.'],
            gaps=['Retry inteligente centralizado pendente.', 'Quality score contínuo pendente.'],
        ...
        IndicadorMaturidade(
            ...
            evidencias=['ADRs, runbooks e gates existem parcialmente.'],
            gaps=['Governance engine ainda não bloqueia todos os desvios operacionais.'],
            proximas_acoes=['Centralizar decisões de bloqueio.', 'Auditar mudanças de maturidade.'],
        ),
        IndicadorMaturidade(
            ...
            proximas_acoes=['Criar health validator por componente.', 'Coletar latência, erro e disponibilidade.'],
        ),
    ]
...
def criar_politicas_base() -> list[PoliticaAutoRemediacao]:
    return [
        PoliticaAutoRemediacao(
            ...
            bloqueios=['não executar se houver falha funcional determinística'],
        ),
        PoliticaAutoRemediacao(
...
def criar_acoes_base() -> list[AcaoAutonoma]:
    return [
        ...
        AcaoAutonoma(
            ...
            resultado_esperado='impedir promoção insegura',
...
def gerar_snapshot_operacao_autonoma(correlation_id: str) -> SnapshotOperacaoAutonoma:
    ...
    return SnapshotOperacaoAutonoma(
        ...
        decisoes_governanca=[
            ...
            'Segurança é gate de promoção e pode bloquear deploy antes de expansão funcional.',
        ],
    )
