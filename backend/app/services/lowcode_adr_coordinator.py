...
ADR_AGENT_CATALOG: list[dict[str, Any]] = [
    ...
    {'adr': 'ADR-0002', 'titulo': 'Segurança e Gates de Produção', 'dominio': 'seguranca', 'agente': 'Agente de Segurança e Governança', 'quando_chamar': 'JWT, CORS, PII/LGPD, secrets, permissões, gates produtivos e revisão de risco.', 'entregaveis': ['matriz de risco', 'guardrails de segurança', 'bloqueios produtivos']},
    {'adr': 'ADR-0003', 'titulo': 'Ambientes dev/hml/prod', 'dominio': 'ambientes', 'agente': 'Agente DevOps de Ambientes', 'quando_chamar': 'Separação de ambiente, promoção, drift, variáveis e configuração por estágio.', 'entregaveis': ['matriz de ambientes', 'plano de promoção', 'evidência de drift']},
    {'adr': 'ADR-0004', 'titulo': 'CI/CD e Qualidade', 'dominio': 'ci_cd', 'agente': 'Agente DevOps/CI', 'quando_chamar': 'Workflows, testes, quality gates, PR, mergeabilidade e evidência de CI verde.', 'entregaveis': ['plano de validação', 'gates obrigatórios', 'evidência de CI']},
    {'adr': 'ADR-0005', 'titulo': 'Observabilidade e Auditoria', 'dominio': 'observabilidade', 'agente': 'Agente de Observabilidade', 'quando_chamar': 'Logs estruturados, correlation_id, auditoria, métricas, rastreabilidade e evidências.', 'entregaveis': ['plano de telemetria', 'eventos auditáveis', 'correlation_id obrigatório']},
    {'adr': 'ADR-0006', 'titulo': 'Analytics Drill-down Schema-driven', 'dominio': 'analytics', 'agente': 'Agente de Analytics e BI', 'quando_chamar': 'Indicadores, drill-down, schema-driven UI, scorecards e análise operacional.', 'entregaveis': ['modelo analítico', 'indicadores', 'drill-down executivo']},
    {'adr': 'ADR-020', 'titulo': 'Connection Broker Permission on Demand', 'dominio': 'integracoes', 'agente': 'Agente de Integrações Corporativas', 'quando_chamar': 'Conectores, permissões sob demanda, APIs externas e integração com Power Platform.', 'entregaveis': ['contrato de integração', 'permissões mínimas', 'plano de fallback']},
    {'adr': 'ADR-021', 'titulo': 'Figma-GitHub Retorno em Tela', 'dominio': 'ux_integrada', 'agente': 'Agente Frontend/UX', 'quando_chamar': 'Experiência visual, retorno em tela, protótipos, responsividade e navegação.', 'entregaveis': ['fluxo de tela', 'critérios UX', 'evidência visual']},
    {'adr': 'ADR-022', 'titulo': 'Autonomous Operations Platform P0.1', 'dominio': 'operacoes_autonomas', 'agente': 'Agente Operacional Autônomo Governado', 'quando_chamar': 'Operação autônoma assistida, triagem, remediação controlada e execução governada.', 'entregaveis': ['plano de execução assistida', 'pontos de autorização humana', 'log operacional']},
    {'adr': 'ADR-023', 'titulo': 'Runtime Health Validator + Remediation', 'dominio': 'runtime', 'agente': 'Agente Runtime Health', 'quando_chamar': 'Healthcheck, remediação, disponibilidade, degradação, rollback e validação pós-deploy.', 'entregaveis': ['health matrix', 'plano de remediação', 'evidência pós-deploy']},
    {'adr': 'ADR-030', 'titulo': 'Governed Dev Automerge', 'dominio': 'governanca_pr', 'agente': 'Agente de Governança de PR', 'quando_chamar': 'Automerge governado, políticas de branch, revisão e controle de mudanças.', 'entregaveis': ['política de merge', 'gates de PR', 'registro de decisão']},
    ...
    {'adr': 'ADR-040', 'titulo': 'Trilhas Padrão Ouro Consolidadas', 'dominio': 'padrao_ouro', 'agente': 'Agente Coordenador de Maturidade Padrão Ouro', 'quando_chamar': 'Consolidação A-E, maturidade, lacunas, Pareto e próximo incremento seguro.', 'entregaveis': ['score de maturidade', 'gaps priorizados', 'próximo incremento']},
    {'adr': 'ADR-041', 'titulo': 'Cofre de Segredos Locais', 'dominio': 'secrets', 'agente': 'Agente de Secrets e Configuração Segura', 'quando_chamar': 'Segredos locais, keyring, criptografia, diagnóstico e fallback seguro.', 'entregaveis': ['matriz de secrets', 'guardrails de cofre', 'diagnóstico seguro']},
    {'adr': 'ADR-USER-FINAL-SHELL', 'titulo': 'User Final Shell', 'dominio': 'usuario_final', 'agente': 'Agente de Produto e Usuário Final', 'quando_chamar': 'Shell visual, jornadas, usabilidade real, navegação final e clareza para usuário.', 'entregaveis': ['jornada do usuário', 'critérios de aceite UX', 'lacunas de produto']},
]
