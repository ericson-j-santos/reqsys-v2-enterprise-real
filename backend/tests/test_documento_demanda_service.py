...
def test_upload_rejeita_tamanho_acima_limite():
    with pytest.raises(ValueError, match='10 MB'):
        validar_upload(
            ...
            conteudo=b'x' * (MAX_UPLOAD_BYTES + 1),
        )
...
def test_classificacao_mantem_revisao_humana():
    ...
    assert {item.tipo for item in candidatos} >= {
        'POSSIVEL_REQUISITO',
        'POSSIVEL_REGRA_NEGOCIO',
    }
...
def test_serializacao_nao_cria_decisao_automatica():
    candidatos = classificar_candidatos('O sistema deve registrar auditoria.')
    payload = json.loads(serializar_candidatos(candidatos))
