...
- `frontend/src/views/PainelIntegracaoView.vue` ja usa `import { api } from '../services/api'` (named import). O build (`npm run build`) e o runtime do Vite passam normalmente; a nota antiga sobre `MISSING_EXPORT "default"` esta resolvida na `main`.
