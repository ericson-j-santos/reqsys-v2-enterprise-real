        ...
        def do_GET(self) -> None:
            ...
            try:
                ...
                if path == "/readyz":
                    store.snapshot()
                    registry.snapshot()
                    self._send(HTTPStatus.OK, {"ok": True, "ready": True})
                    return
                if path == "/v1/status":
                ...
                if path == "/v1/workers":
                    ...
                    return
                worker_prefix = "/v1/workers/"
                if path.startswith(worker_prefix) and path.endswith("/assignments"):
                    worker_id = path[len(worker_prefix):-len("/assignments")].strip("/")
                    if not worker_id:
                        raise KeyError(worker_id)
                    store.recover_expired_leases()
                    self._send(
                        HTTPStatus.OK,
                        {"items": [asdict(item) for item in registry.assigned_items(worker_id)]},
                    )
                    return
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                if path.startswith(prefix):
                    ...
                    if action == "renew":
                        item = store.renew_lease(
                            item_id,
                            worker_id=body.get("worker_id", ""),
                            lease_seconds=body.get("lease_seconds", 120),
                        )
                        self._send(HTTPStatus.OK, {"item": self._item_payload(item)})
                        return
                    if action == "complete":
