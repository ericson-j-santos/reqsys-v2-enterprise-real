...
const servidor = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/oauth/token') {
      ...
      return responder(res, 200, {access_token: 'token-sintetico', token_type: 'Bearer', expires_in: 300});
    }

    if (req.method === 'POST' && req.url === '/v1/dossies') {
      const corpo = await lerJson(req);
      if (req.headers.authorization !== 'Bearer token-sintetico') return responder(res, 401, {codigo: 'NAO_AUTORIZADO'});
