...
def build_lifecycle_index(retention_index: dict[str, Any]) -> dict[str, Any]:
    ...
    for artifact in artifacts:
        ...
        state = "LIFECYCLE_REVIEW_ONLY_READY" if has_hash and has_retention else "LIFECYCLE_INCOMPLETE"
        if state != "LIFECYCLE_REVIEW_ONLY_READY":
...
def main() -> int:
    ...
    payload = build_lifecycle_index(load_json(report_dir / REQUIRED_RETENTION_INDEX))
    (report_dir / "product-intelligence-evidence-navigation-governance-lifecycle-index.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (report_dir / "product-intelligence-evidence-navigation-governance-lifecycle-index.md").write_text(render_markdown(payload), encoding="utf-8")
    print(f"Evidence navigation governance lifecycle index: {payload['readiness_state']} risk={payload['risk_percent']}%")
