...
def evaluate_gate(recovery_index: dict[str, Any]) -> dict[str, Any]:
    ...
    if recovery_index.get("automatic_execution_authorized") is not False:
        blockers.append({"code": "AUTO_EXECUTION_ENABLED", "severity": "critical", "detail": "Automatic execution must remain disabled"})
    if recovery_index.get("destructive_action_authorized") is not False:
        blockers.append({"code": "DESTRUCTIVE_ACTION_ENABLED", "severity": "critical", "detail": "Destructive action must remain disabled"})
    if recovery_index.get("production_promotion_authorized") is not False:
        blockers.append({"code": "PRODUCTION_PROMOTION_ENABLED", "severity": "critical", "detail": "Production promotion must remain disabled"})
