...
def classify(coverage: float, risk: float) -> tuple[str, str]:
...
def build_payload(final_index: dict[str, Any]) -> dict[str, Any]:
    evidence = final_index.get("evidence", [])
        ...
        cards.append(
            {
                ...
                "available": available,
                "state": "CONSOLIDATED" if available else "MISSING",
                "status_color": "green" if available else "red",
                "anchor": artifact.replace(".", "-").replace("_", "-").lower(),
            }
        )

    coverage = float(final_index.get("evidence_coverage_percent", 0.0))
    risk = float(final_index.get("risk_percent", 100.0))
    readiness_color, readiness_label = classify(coverage, risk)
    ...
    return {
        ...
        "readiness_state": final_index.get("readiness_state", "UNKNOWN"),
        "readiness_color": readiness_color,
        "readiness_label": readiness_label,
        "evidence_coverage_percent": coverage,
        "confidence_percent": final_index.get("confidence_percent", 0),
        "risk_percent": risk,
...
def render_html(payload: dict[str, Any]) -> str:
    nav = "\n".join(
        f'<a href="#{html.escape(card["anchor"])}" class="nav-item {html.escape(card["status_color"])}">{html.escape(card["artifact"])}</a>'
        for card in payload["cards"]
    )
    cards = "\n".join(
        f'''<section id="{html.escape(card['anchor'])}" class="card {html.escape(card['status_color'])}">
  <div class="card-header"><strong>{html.escape(card['artifact'])}</strong><span>{html.escape(card['state'])}</span></div>
...
<body>
<header><h1>Product Intelligence Evidence Navigation UI</h1><p>{html.escape(payload['readiness_label'])} · review_only · revisão humana obrigatória</p></header>
<main>
<nav aria-label="Mapa navegável de evidências">{nav}</nav>
<section>
<div class="summary">
...
def main() -> int:
    ...
    payload = build_payload(load_json(report_dir / "product-intelligence-final-evidence-index.json"))
    (report_dir / "product-intelligence-evidence-navigation-ui.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (report_dir / "product-intelligence-evidence-navigation-ui.html").write_text(render_html(payload), encoding="utf-8")
    (report_dir / "product-intelligence-evidence-navigation-ui.md").write_text(render_markdown(payload), encoding="utf-8")
    print(f"Evidence navigation UI: {payload['readiness_color']} coverage={payload['evidence_coverage_percent']}% available={payload['available_count']}/{payload['artifact_count']}")
    return 0
