...
def main() -> int:
    ...
    (report_dir / "product-intelligence-evidence-navigation-artifact-manifest.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    (report_dir / "product-intelligence-evidence-navigation-artifact-manifest.md").write_text(
        render_markdown(payload),
        encoding="utf-8",
    )

    print(
        "Evidence navigation artifact publisher: "
        f"{payload['readiness_state']} "
        f"coverage={payload['artifact_coverage_percent']}% "
