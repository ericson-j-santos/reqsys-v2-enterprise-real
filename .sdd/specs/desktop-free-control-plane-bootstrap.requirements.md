...
8.3. O reinício por estado `offline` não se aplica a labels divergentes ou processo não verificável.
8.4. Se o contrato local existir mas o registro GitHub estiver ausente, o bootstrap deve obter remove-token e registration-token efêmeros antes de qualquer mutação, parar somente o listener do `runner_home` governado, executar `config.cmd remove --token`, registrar novamente com nome/labels fixos e revalidar o registro; falha de token/permissão deve manter o fluxo fail-closed.
...
- Registro GitHub ausente com contrato local presente deve acionar uma única tentativa governada de reconfiguração; remove-token e registration-token permanecem somente em memória e são obtidos antes de parar o listener.
