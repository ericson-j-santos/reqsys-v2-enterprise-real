...
32. Após evidência de rejeição já em `minimal_create`, o bootstrap progressivo deve iniciar por uma fase `fabric_documented_minimal` alinhada à estrutura pública de exemplo do Microsoft Fabric (RDL 2016, header `rd/df`, `AutoRefresh`, `ReportSections` com `Body/Width/Page` e `ReportParametersLayout` sem parâmetros), seguida por `generated_minimal -> datasource -> dataset -> full`. Cada fase aceita deve ser comprovada por `getDefinition` independente antes da próxima mutação.
