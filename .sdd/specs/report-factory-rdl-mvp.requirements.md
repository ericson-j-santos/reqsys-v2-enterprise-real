...
32. Cada fase aceita do bootstrap progressivo deve ser confirmada por `getDefinition` independente e igualdade SHA-256 com o RDL daquela fase antes de avançar; falha de leitura ou hash deve interromper o fluxo e registrar a fase como rejeitada.
33. Após o E2E `35883428772` comprovar rejeição em `minimal_create`, as variantes reduzidas `minimal` e `datasource` devem preservar um `ReportParametersLayout/GridLayoutDefinition` vazio, com dimensões positivas e posicionado após `ReportSections`, conforme a forma do exemplo público atual do Fabric; `AuthoringMetadata` permanece opcional e não deve ser tratado como requisito funcional sem nova evidência.
34. Após o E2E `35885337589` manter `minimal_create` bloqueado mesmo com layout vazio, a fase `minimal` deve deixar de ser uma redução do RDL do ReqSys e passar a ser um controle positivo independente, alinhado à forma pública documentada pelo Fabric (RDL 2016, cabeçalho, seção, textbox, footer, margens e `ReportParametersLayout` vazio). As fases seguintes devem adicionar somente a camada sob teste antes de avançar.
