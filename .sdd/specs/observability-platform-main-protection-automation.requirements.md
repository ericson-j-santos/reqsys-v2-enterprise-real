...
9. A execução captura o SHA corrente de `observability-platform/main`, comprova `test=completed/success`, relê o mesmo SHA imediatamente antes do PUT e exige o mesmo SHA depois da mutação.
10. A proteção exige status check `test` com strict=true, Pull Request, enforcement para administradores e bloqueio de force-push e exclusão.
...
- O modo `apply-observability-platform` executa somente no Noteri allowlisted.
- O SHA alvo é capturado em runtime e permanece idêntico antes/depois do PUT.
- O check real `test` precisa estar verde no SHA capturado.
- A proteção final exige Pull Request + `test`, aplica a administradores e bloqueia force-push/exclusão.
