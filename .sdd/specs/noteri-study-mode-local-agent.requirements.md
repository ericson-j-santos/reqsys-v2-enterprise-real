...
O frontend deve montar a interface antes de iniciar autenticação silenciosa externa e sem bloquear o mount em `router.isReady()`. Uma falha ou demora do MSAL não pode manter o usuário em tela branca; deep links devem preservar o comportamento de foco acessível da navegação inicial, e a rota protegida deve permanecer no login e retornar ao destino original após autenticação válida.
...
11. O app monta antes do bootstrap MSAL assíncrono, não aguarda `router.isReady()` antes do mount e mantém a proteção das rotas privadas pelo router guard.
