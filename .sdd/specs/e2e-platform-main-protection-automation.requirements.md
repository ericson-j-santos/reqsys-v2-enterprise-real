...
Aplicar proteção real na branch `main` de `ericson-j-santos/e2e-platform` com custo adicional zero, sem interface gráfica e sem depender de segredo GitHub hospedado no Actions.
...
3. O Authorized Actions Gateway continua expondo somente o comando literal `/reqsys run protect-e2e-platform-main`.
4. O job de aplicação executa no runner `[self-hosted, Windows, X64, noteri, reqsys-dev]` e exige host exato `Noteri`.
...
8. A autenticação GitHub deve vir exclusivamente do perfil local já existente do `gh` no Noteri; `GH_TOKEN` e `GITHUB_TOKEN` são removidos do ambiente dos subprocessos e nenhum valor de credencial pode ser lido, impresso ou persistido.
9. A execução captura o SHA corrente de `e2e-platform/main`, comprova `contract-self-test=completed/success`, relê o mesmo SHA imediatamente antes do PUT e exige o mesmo SHA depois da mutação.
10. A proteção exige status check `contract-self-test` com strict=true, Pull Request, enforcement para administradores e bloqueio de force-push e exclusão.
...
- O modo `apply-e2e-platform` executa somente no Noteri allowlisted.
- O SHA alvo é capturado em runtime e permanece idêntico antes/depois do PUT.
- O check real `contract-self-test` precisa estar verde no SHA capturado.
- A proteção final exige Pull Request + `contract-self-test`, aplica a administradores e bloqueia force-push/exclusão.
