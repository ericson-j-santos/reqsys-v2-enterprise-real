...
7. Se o arquivo existir, for legível, não vazio e autenticar o runtime, deve ser reutilizado sem rotação. Se houver `401`, nenhum segredo novo deve ser gerado apenas por mismatch: a reconciliação deve primeiro validar o token já montado no runtime.
...
12. Quando um `401` comprovar mismatch e a comparação em memória provar que o token montado no container diverge do arquivo host canônico, o token do runtime deve ser testado independentemente em `/v1/snapshot` antes de qualquer mutação. Se esse token já autenticar corretamente, o arquivo host deve ser ressincronizado atomicamente para o valor ativo do runtime, sem registrar o valor, sem recriar/reiniciar o serviço e sem gerar segredo novo. O arquivo deve ser relido e comparado em memória após a gravação. Se host e container já contiverem o mesmo token e a API ainda responder `401`, falhar fechado como `worker_pool_auth_process_mismatch`. Recriação Compose fica restrita ao caso em que um token novo precisou ser criado porque o arquivo canônico estava ausente ou vazio; nesse caso permanecem `--force-recreate --no-deps --no-build`, validação de source canônico e fail-closed.
13. A evidência pode informar somente rotação/reuso, recriação do serviço, ressincronização do bind mount, reutilização do token ativo do runtime, ressincronização do arquivo host, HTTP status, readback e `reason` sanitizado de readiness. Respostas HTTP 503 podem ser lidas apenas para classificar, sem registrar token, caminho sensível ou valor de variável. Devem existir reason codes distintos para endpoint indisponível, arquivo de auth não visível no container, `EXPECTED_RULES_SHA` ausente e mismatch de token; `secret_value_exposed=false` é obrigatório.
...
- testes unitários positivos e negativos do restaurador, incluindo criação segura do diretório pai canônico, reconciliação host←runtime quando o token montado autentica, controle que mantém o host intacto quando o token do runtime não autentica, controle que falha quando host/container já coincidem, fallback Compose somente para criação de token novo, recuperação controlada de compose source, preservação sanitizada do payload 503 e reason codes específicos de readiness, e da allowlist Risk3 temporária verdes;
